...
// KPI Query: Get total revenue, units sold, and PO count from active POs
$kpi_sql = "
    SELECT
...
// On-Time Delivery KPI from active POs
$delivery_sql = "
    SELECT
...
// Top 5 Best-Selling Products from active POs
$top_products_sql = "
...
// *** NEW: Query for Cancelled POs ***
$cancelled_po_sql = "
    SELECT
        COALESCE(SUM(pi.qty * pi.price), 0) AS total_cancelled_value,
        COUNT(DISTINCT pl.po_id) AS total_cancelled_count
    FROM po_list pl
    JOIN po_items pi ON pl.po_id = pl.po_id
    WHERE pl.po_date BETWEEN ? AND ?
      AND pl.status = 'deactivated'
";
$stmt = $conn->prepare($cancelled_po_sql);
$stmt->bind_param('ss', $start_date, $end_date);
$stmt->execute();
$cancelled_po_data = $stmt->get_result()->fetch_assoc();
$stmt->close();


// Sales by Customer (for Chart) from active POs
$customer_sales_sql = "
...
$stmt->bind_param('ss', $start_date, $end_date);
$stmt->execute();
$customer_sales = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
$stmt->close();

// Prepare data for Chart.js
$chart_labels = json_encode(array_column($customer_sales, 'cus_name'));
$chart_data = json_encode(array_column($customer_sales, 'revenue'));
        ...
        <div class="row">
            <div class="col-xl-7 mb-4">
                <div class="card shadow-sm h-100">
                    <div class="card-header"><h5 class="mb-0">ยอดขายของแต่ละลูกค้า</h5></div>
                    <div class="card-body" style="min-height: 400px;"><canvas id="salesChart"></canvas></div>
                </div>
            </div>
            <div class="col-xl-5 mb-4">
                <div class="card shadow-sm mb-4">
                    <div class="card-header"><h5 class="mb-0">5 อันดับสินค้าขายดี</h5></div>
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-striped table-hover mb-0">
                                <thead><tr><th>สินค้า</th><th class="text-end">ยอดขาย</th></tr></thead>
                                <tbody>
                                    ...
                                    <?php endforeach; if(empty($top_products)) echo '<tr><td colspan="2" class="text-center text-muted">ไม่มีข้อมูล</td></tr>'; ?>
                                </tbody>
                            </table>
                        </div>
                    </div>
                </div>
                <!-- *** MAJOR CHANGE: Replaced "Top Customers" with "Cancelled POs" *** -->
                <div class="card shadow-sm">
                    <div class="card-header bg-danger text-white">
                        <h5 class="mb-0"><i class="bi bi-x-octagon-fill me-2"></i>ยอดรวมใบสั่งซื้อที่ยกเลิก</h5>
                    </div>
                    <div class="card-body text-center">
                        <h3 class="card-title text-danger fw-bolder">฿<?= number_format($cancelled_po_data['total_cancelled_value'] ?? 0, 2) ?></h3>
                        <p class="card-text text-muted mb-0">
                            จากทั้งหมด <strong><?= number_format($cancelled_po_data['total_cancelled_count'] ?? 0) ?></strong> ใบที่ถูกยกเลิก
                        </p>
                    </div>
                </div>
            </div>
