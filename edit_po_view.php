...
// 2. รวมไฟล์เชื่อมต่อฐานข้อมูล และ navbar
include_once __DIR__ . '/config_db.php';
include_once __DIR__ . '/navbar.php';

// 3. ดึงข้อมูลส่วนหัวของ PO และชื่อลูกค้าใน Query เดียว
$stmt = $conn->prepare(
    "SELECT pl.po_id, pl.po_date, pl.cus_id, c.cus_name 
     FROM po_list pl
...
// 4. ดึงรายการสินค้าทั้งหมดของ PO นี้
$stmt = $conn->prepare(
    "SELECT 
...
$stmt->bind_param('i', $header['po_id']);
$stmt->execute();
...
<head>
    ...
    <style>
        ...
        /* Make read-only inputs look like normal text */
...
<body>
    ...
    <div class="container my-5">
        <div class="card shadow-lg">
            <div class="card-header bg-warning bg-gradient text-dark">
                <h4 class="mb-0"><i class="bi bi-pencil-square me-2"></i>Edit Purchase Order: <?= htmlspecialchars($poNo) ?></h4>
            </div>
                            ...
                            <table class="table table-hover table-bordered align-middle">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <input type="number" name="items[<?= $i ?>][qty]" class="form-control text-end" min="1" value="<?= $item['qty'] ?>" required>
                                        </td>
                                        <td>
                                            <input type="text" class="form-control-plaintext text-end" value="<?= number_format($item['price'], 2) ?>" readonly>
                                        </td>
                                        <td>
                                            <input type="text" name="items[<?= $i ?>][delivery_date]" class="form-control date-picker" value="<?= htmlspecialchars($item['delivery_date']) ?>" required>
                                        </td>
                                        <td>
                                            <input type="text" name="items[<?= $i ?>][actual_delivery_date]" class="form-control date-picker" value="<?= htmlspecialchars($item['actual_delivery_date']) ?>">
                                        </td>
                                    </tr>
                                    <?php endforeach; ?>
                                </tbody>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id="confirm-update-btn" class="btn btn-primary">Yes, Update</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
    $(function(){
        // Initialize all date pickers on the page with common settings
        $('.date-picker, #po_date').daterangepicker({
        ...
        // --- Form Submission Logic with Confirmation ---
        
        // 1. When the main "Update" button is clicked, show the confirmation modal.
        $('#submit-btn').on('click', function() {
            const confirmationModal = new bootstrap.Modal(document.getElementById('confirmationModal'));
            confirmationModal.show();
        });

        // 2. When the "Yes, Update" button inside the modal is clicked, submit the form.
